...
public:
    ...
    // Reinicializa la pila como vacía
    void clear() {
        while (!estaVacia()) {
            pop();
        }
...
public:
    ...
    // Destructor para liberar la memoria de val si es necesario
    ~tNodoAux(){
        delete val;
    }

    // Métodos para acceder y modificar val
    Pelicula* getVal() const { return val; }
    void setVal(Pelicula* pelicula) { val = pelicula; }

    // Métodos para acceder y modificar sig
...
tNodoAux* Director::obtenerpelis() {
    ...
    while (aux != nullptr) {
        Pelicula* copia = new Pelicula;  // Copiar la película
        *copia = *aux->val;

        tNodoAux* nuevo_nodo = new tNodoAux(copia); // Crear nuevo nodo con copia

        if (head_temp == nullptr) {
            head_temp = tail_temp = nuevo_nodo;
        } else {
            tail_temp->setSig(nuevo_nodo);  // Usar setSig para actualizar el puntero siguiente
    ...
    if (head_temp != nullptr) {
        head_temp->moveToStart(); // Reinicia la posición de head_temp al inicio
    }
...
void Director::liberarpelis(tNodoAux* lista) {
    ...
    while (actual != nullptr) {
        ...
        actual = actual->getSig();  // Usar getSig para avanzar al siguiente nodo
        delete temp;  // Liberar cada nodo, el destructor de tNodoAux libera `val`
    }
    actual = nullptr;
}
...
void Arboles::copiar_arbol_aux(aNodo* nodo) {
    ...
    while (actual != nullptr) {
        Pelicula* nueva_pelicula = new Pelicula(*actual->getVal()); // Suponiendo que tienes un constructor de copia
...
Director * Arboles::buscar_director( string director){
    ...
    while(aux != nullptr){
        // cout<<"nombre:"<<director<<endl;
        // cout<<"nombre;"<<aux->val->obtener_nombre()<<endl;
        // cout<<aux->val->obtener_nombre()<<endl;
        if (aux->val->obtener_nombre() == director) {
            /* cout<<"retonar director encontrado"<<endl; */
...
void Arboles::mostrardirector(string nombre) {
    ...
    /* cout<<"entra a la funcion mostrardirector"<<endl; */
    // Verifica si se encontró el director
    if (directorEncontrado != nullptr) {
        /* cout<<"entra al if si encuentra al director"<<endl; */
...
void Arboles::inordenYGuardarEnPila(aNodo* nodo, Pila *p) {
    if (nodo == nullptr) return;
    inordenYGuardarEnPila(nodo->izq, p ); // Recorrer el subárbol izquierdo
    p->push(nodo->val->obtener_nombre()); // Guardar el valor del nodo en el arreglo
    inordenYGuardarEnPila(nodo->der, p); // Recorrer el subárbol derecho
}
